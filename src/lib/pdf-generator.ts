...
export function generateMedicalRecordPDF(record: MedicalRecord, patient: Patient): void {
  ...
  complaintLines.forEach((line: string) => addText(line, 5))
  doc.setFontSize(12)

  addText('Diagnosis:', 5)
  doc.setFontSize(12)
  const diagnosisLines = doc.splitTextToSize(record.diagnosis, 170)
  diagnosisLines.forEach((line: string) => addText(line, 5))
  doc.setFontSize(12)

  addText('Clinical Notes:', 5)
  doc.setFontSize(12)
  const notesLines = doc.splitTextToSize(record.notes, 170)
  notesLines.forEach((line: string) => addText(line, 5))
  yPos += lineHeight
  doc.setFontSize(12)
  ...
  // Treatment Points Summary with improved formatting
   if (record.treatmentPoints && record.treatmentPoints.length > 0) {
    // Check if there's enough space for all points plus title and header background
    const requiredSpace = (record.treatmentPoints.length + 1) * lineHeight + 8
    ...
    // Set text color to black and font to bold before the title
    ...
    // Add the title
    addText('Treatment Points:',5)
    
    // Reset font to normal for the points
    doc.setFont('helvetica', 'normal')
